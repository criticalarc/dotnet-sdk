...
namespace CheckInSummaryReporting
{
    internal class Program
    {
        ...
        private static readonly XServerId SafeZoneId = (XServerId) "criticalarc.com";
            ...
            Task.Run(async () =>
                {
                    ...
                    using (var client = new SafeZoneClient(new SafeZoneClientSettings(UserName, Password)))
                    {
                        ...
                        var results = await client.CheckInClient.GetSessionsAsync(sessionsQuery, MaxResultsPerPage,
                            AsyncOptions.None);
                        ...
                        await results.ForEachPageAsync(async page =>
                            {
                                foreach (var session in page)
                                {
                                    if (!regionNames.TryGetValue(session.RegionId, out var regionName))
                                        try
                                        {
                                            var region = await client.CoreClient.GetRegionAsync(SafeZoneId,
                                                session.RegionId, AsyncOptions.None);
                                            regionNames[session.RegionId] = regionName =
                                                XSafeZoneRegionFields.DisplayName.GetValueOrDefault(region) ??
                                                (string) region.Id;
                                        }
                                    ...
                                    if (!userNames.TryGetValue(session.UserId, out var userName))
                                        try
                                        {
                                            var user = await client.UserClient.GetUserAsync(SafeZoneId, session.UserId,
                                                AsyncOptions.None);
                                    ...
                                    if (session.ExitedTimestamp == null)
                                        Console.WriteLine(
                                            $"{userName} is currently checked in at {regionName} since {session.EnteredTimestamp.Value.ToLocalTime():g}");
                                    else
                                        Console.WriteLine(
                                            $"{userName} was checked in at {regionName} between {session.EnteredTimestamp.Value.ToLocalTime():g} and {session.ExitedTimestamp.Value.ToLocalTime():g}");
                                }
                            },
                            AsyncOptions.None);
                    }
